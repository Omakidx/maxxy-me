...
Use the login command generated by **Setup -> Codex** to authenticate one lane at a time. Keep the host agent running so it can report the new ready status. Do not copy another lane's credential directory over it.
