...
Use these tools from `.maxxy-agent/tools/` when working on CodeRabbit tasks:

| Tool | When to Use |
|------|-------------|
| `.maxxy-agent/tools/git.md` | PR workflows, branching, conventional commits that CodeRabbit reviews |
| `.maxxy-agent/tools/config-generator.md` | GitHub Actions CI for CodeRabbit CLI, ESLint/Prettier configs to reference in instructions |
| `.maxxy-agent/tools/code-quality.md` | Complement CodeRabbit with local quality metrics, coverage thresholds |
| `.maxxy-agent/tools/security-scanner.md` | OWASP checks to encode as path_instructions for security-critical paths |
| `.maxxy-agent/tools/cli-productivity.md` | Shell aliases for `cr` commands, tmux for agent review loops |
| `.maxxy-agent/tools/test-scaffolder.md` | Test patterns that CodeRabbit should check for in `**/*.test.*` path instructions |
